//! The Attack Rating calc engine. Pure (no I/O): given a weapon, up to three
//! gems, and the character's stats, it reproduces the spreadsheet's per-type AR.

use crate::types::{ArBreakdown, ConvertedElement, Gem, Stats, Weapon, WeaponType};
